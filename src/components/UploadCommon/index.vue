  ...
  <div class="attach-box">
    ...
    >
      <template v-if="listType === 'text'">
        <el-button style="float: left,color:#1D549D"  type="primary" round>
          {{ fileList.length > 0 ? "继续" + upLoadBtnTxt : upLoadBtnTxt }}
        </el-button>
        <div class="tips" v-if="uploadTips">
          支持{{
            accept === "image/*"
        ...
        <!-- <el-button  type="primary">{{upLoadBtnTxt}}</el-button> -->
      </template>
...
export default {
  ...
  data() {
    return {
      ...
      // 默认配置项
      BaseHref: window.g.api,
      baseShowHref: window.g.api,
    };
  },
  props: {
    action: {
