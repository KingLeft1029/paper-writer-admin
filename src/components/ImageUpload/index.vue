<template>
  <div>
    <el-upload class="upload-demo" :action="uploadImgUrl" list-type="picture" :on-success="handleUploadSuccess"
      ...
      :class="{ hide: (isReadonly || this.fileList.length >= this.limit) && this.fileList.length }"
      :disabled="isReadonly">
      <el-button  class="elButtonStyle"><i class="el-icon-upload2 elIconUpload"></i>上传</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      uploadImgUrl: window.g.api + "/common/upload", // 上传的图片服务器地址
      headers: {
