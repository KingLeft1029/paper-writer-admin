 ...
 * @LastEditTime: 2022-04-27 16:47:54
...
<script>
import { outDetailPage } from "@/api/trading";
...
export default {
  ...
  methods: {
    ...
    handleExport() {
      this.$confirm("是否确认导出所选商家管理平台端数据项?", "警告", {
        ...
        .then(() => {
          ...
          this.download(
            "/orderInfo/recordBack/export",
            params,
            `交易明细(出账)_${new Date().getTime()}.xlsx`
          );
        })
        .then((response) => {
    ...
    getList() {
      ...
      outDetailPage(params).then((response) => {
